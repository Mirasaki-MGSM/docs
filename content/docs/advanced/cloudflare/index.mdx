...
<Steps>
    ...
    <Step>
        ...
        | A | @ | `1.1.1.1` | Auto | Points the root domain to your MGSM instance, use any subdomain instead of `@` if you are using a subdomain for the web/frontend. |
        | A | api | `1.1.1.1` | Auto | Points the API subdomain to your MGSM instance API, use a different `name` if you would like to use a different subdomain. |
        | A | cms | `1.1.1.1` | Auto | Points the CMS subdomain to your MGSM instance CMS, use a different `name` if you would like to use a different subdomain. |

        <Callout type="info" className="-mt-2 mb-4">
            Please note that `1.1.1.1` is a placeholder IP address, you will obtain the correct IP address during the onboarding process. **Disable the "Proxy Status" checkbox for all records except the `www` record.** If there is any issues with your configuration, we will notify you when we try to verify the domain.
        </Callout>
